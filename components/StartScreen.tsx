...
const RankingDisplay: React.FC<{ title: string; ranking: RankingEntry | null }> = ({ title, ranking }) => {
    ...
    return (
        <div className="mb-6 p-4 border border-yellow-400/50 rounded-lg bg-black/30 w-full max-w-md">
          ...
          <div className="flex flex-wrap justify-around text-lg gap-x-4 gap-y-1">
            ...
            <span className="text-white">{t('coins_earned')}: <span className="font-bold text-yellow-400">{ranking.coinsEarned}</span></span>
          </div>
        </div>
    );
};

const StartScreen: React.FC<StartScreenProps> = ({ onStart, onOpenSettings, onOpenStore, bestRanking, lastRunStats, controlMode, isFullscreen, onToggleFullscreen }) => {
  const { t } = useTranslation();
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center p-8 bg-black/50 rounded-lg shadow-2xl shadow-cyan-500/20 border-2 border-cyan-400">
      ...
      <div className="flex flex-col gap-4 w-full max-w-xs">
        ...
        <button
            onClick={onOpenStore}
            className="w-full px-10 py-3 bg-yellow-600 text-white font-bold text-xl rounded-md border-2 border-yellow-400
        ...
        <div className="flex gap-4">
            <button
                ...
                className="flex-grow px-10 py-3 bg-gray-700 text-white font-bold text-xl rounded-md border-2 border-gray-500
            ...
            <button
                ...
                className="w-16 h-auto p-3 bg-gray-700 text-white rounded-md border-2 border-gray-500
                           hover:bg-purple-600 hover:border-purple-400 hover:scale-105 transition-all duration-300
                           flex items-center justify-center"
            >
                {isFullscreen 
                    ? <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M8 3v3a2 2 0 0 1-2 2H3m18 0h-3a2 2 0 0 1-2-2V3m0 18v-3a2 2 0 0 1 2-2h3M3 16h3a2 2 0 0 1 2 2v3" /></svg>
                    : <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M8 3H5a2 2 0 0 0-2 2v3m18 0V5a2 2 0 0 0-2-2h-3m0 18h3a2 2 0 0 0 2-2v-3M3 16v3a2 2 0 0 0 2 2h3" /></svg>
                }
            </button>
        </div>
      </div>
