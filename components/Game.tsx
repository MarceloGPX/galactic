...
import { Player, Enemy, Projectile, Boss, Explosion, Vector2D, GameObject, SkillId, Skill, EnemyProjectile, SpecialAbilityId, Helper, Difficulty, Quality, RankingEntry, ControlMode, Collectible } from '../types';
...
import { CollectibleVisual } from './renderers/CollectibleVisual';
...
interface GameState {
  ...
  nextRevivalKeySpawnTime: number;
}
...
const RevivalScreen: React.FC<{ timeLeft: number; onRevive: () => void; onDecline: () => void; }> = ({ timeLeft, onRevive, onDecline }) => {
    const { t } = useTranslation();
    const secondsLeft = (timeLeft / 1000).toFixed(1);
    const progress = (timeLeft / C.REVIVE_SCREEN_DURATION_MS) * 100;
    
    return (
        <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-50 backdrop-blur-md p-4">
            <div className="text-center p-8 bg-black/50 rounded-lg shadow-2xl shadow-yellow-500/20 border-2 border-yellow-400 flex flex-col gap-6 w-96">
                 <h2 className="text-5xl font-bold text-yellow-400 tracking-widest" style={{ textShadow: '0 0 15px #facc15' }}>{t('revive_title')}</h2>
                <p className="text-xl text-yellow-100">{t('revive_prompt')}</p>
                
                <div className="w-full bg-gray-700 rounded-full h-4 border-2 border-gray-500 overflow-hidden my-2">
                    <div className="bg-yellow-400 h-full rounded-full" style={{ width: `${progress}%`, transition: 'width 0.1s linear' }} />
                </div>

                <button onClick={onRevive} className="w-full px-10 py-4 bg-yellow-500 text-black font-bold text-2xl rounded-md border-2 border-yellow-300 hover:bg-yellow-300 hover:text-black hover:scale-105 transition-all duration-300 shadow-[0_0_20px_rgba(250,204,21,0.7)]">
                    {t('revive_button')}
                </button>
                <button onClick={onDecline} className="w-full px-10 py-3 bg-gray-600 text-white font-bold text-xl rounded-md border-2 border-gray-400 hover:bg-red-700 hover:border-red-500 hover:scale-105 transition-all duration-300">
                    {t('give_up_button')}
                </button>
            </div>
        </div>
...
const Game: React.FC<GameProps> = ({ onGameOver, onRestart, onGoHome, difficulty, quality, controlMode }) => {
  ...
  const [gameState, setGameState] = useState<GameState>({
    ...
    nextRevivalKeySpawnTime: C.REVIVAL_KEY_SPAWN_INTERVAL_MS,
  });
  const [isPaused, setIsPaused] = useState(false);
  const [isLevelingUp, setIsLevelingUp] = useState(false);
  const [isReviving, setIsReviving] = useState(false);
  const [revivalTimeLeft, setRevivalTimeLeft] = useState(C.REVIVE_SCREEN_DURATION_MS);
  ...
  const triggerGameOver = useCallback(() => {
    const { score, phase, gameTime, player } = gameStateRef.current;
      ...
      switch (id) {
        case 'bomb':
          ...
          nextState.enemies.forEach(e => {
              if (e.type === 'normal') { scoreToAdd += C.ENEMY_NORMAL_SCORE; xpToAdd += C.ENEMY_NORMAL_SCORE; coinsToAdd += C.COIN_REWARD_NORMAL; }
              else if (e.type === 'fast') { scoreToAdd += C.ENEMY_FAST_SCORE; xpToAdd += C.ENEMY_FAST_SCORE; coinsToAdd += C.COIN_REWARD_FAST; }
              else { scoreToAdd += C.ENEMY_STRONG_SCORE; xpToAdd += C.ENEMY_STRONG_SCORE; coinsToAdd += C.COIN_REWARD_STRONG; }
          ...
          newPlayerState.coins += coinsToAdd;
  ...
  const handleUseRevivalKey = useCallback(() => {
    setIsReviving(false);
    setGameState(prev => {
        const newPlayerState = { ...prev.player };
        newPlayerState.revivalKeys--;
        newPlayerState.hull = newPlayerState.maxHull * C.REVIVE_HULL_RESTORE_PERCENTAGE;
        newPlayerState.shield = newPlayerState.maxShield;
        newPlayerState.invincibleUntil = Date.now() + C.REVIVE_INVINCIBILITY_DURATION_MS;
  ...
  const gameLoop = useCallback(() => {
    if (isLevelingUp || isReviving) { gameLoopRef.current = requestAnimationFrame(gameLoop); return; }
    ...
    setGameState(prev => {
      let { player, enemies, projectiles, enemyProjectiles, collectibles, boss, explosions, score, phase, gameTime, maxEnemies, enemySpawnRateMultiplier, enemyFireRateMultiplier, helper, nextBossSpawnTime, nextRevivalKeySpawnTime } = { ...prev };
      player = { ...player, position: { ...player.position } }; enemies = [...enemies]; projectiles = [...projectiles]; enemyProjectiles = [...enemyProjectiles]; explosions = [...explosions]; collectibles = [...collectibles];
      boss = boss ? { ...boss, position: { ...boss.position }, velocity: {...boss.velocity} } : null; helper = helper ? { ...helper, position: { ...helper.position } } : null;

      let scoreToAdd = 0, xpToAdd = 0, coinsToAdd = 0; gameTime += deltaTime;
      ...
      collectibles = collectibles.map(c => ({...c, position: { x: c.position.x + c.velocity.x, y: c.position.y + c.velocity.y }})).filter(c => c.position.y < C.GAME_HEIGHT);
      
      if (isSuperShieldActive) {
          const attractionStrength = 0.2; // Strong attraction
          enemyProjectiles.forEach(p => {
              const playerCenterX = player.position.x + player.size.width / 2;
              const playerCenterY = player.position.y + player.size.height / 2;
              const dirX = playerCenterX - p.position.x;
              const dirY = playerCenterY - p.position.y;
              const dist = Math.sqrt(dirX * dirX + dirY * dirY);

              if (dist > 1) { // Avoid division by zero and extreme force
                  const normDirX = dirX / dist;
                  const normDirY = dirY / dist;
                  const speed = Math.sqrt(p.velocity.x**2 + p.velocity.y**2);

                  p.velocity.x = p.velocity.x * (1 - attractionStrength) + normDirX * speed * attractionStrength;
                  p.velocity.y = p.velocity.y * (1 - attractionStrength) + normDirY * speed * attractionStrength;
                  
                  const newSpeed = Math.sqrt(p.velocity.x**2 + p.velocity.y**2);
                  if (newSpeed > 0) {
                      const speedCorrection = speed / newSpeed;
                      p.velocity.x *= speedCorrection;
                      p.velocity.y *= speedCorrection;
                  }
              }
          });
      }
      ...
      for (let i = projectiles.length - 1; i >= 0; i--) {
        ...
        if (boss && checkCollision(p, boss)) {
          ...
          if (boss.health <= 0) { 
              const coinReward = C.BOSS_BASE_COIN_REWARD + Math.floor((phase - 1) / 5) * C.BOSS_PHASE_COIN_BONUS;
              scoreToAdd += C.BOSS_SCORE_REWARD; xpToAdd += C.BOSS_SCORE_REWARD; coinsToAdd += coinReward; 
              createExplosion(boss.position, boss.size); boss = null; bossSpawned.current = false; phase++; maxEnemies *= C.PHASE_MAX_ENEMIES_MULTIPLIER; if (phase >= 10) nextBossSpawnTime = gameTime + C.BOSS_SPAWN_INTERVAL_MS; 
          }
      ...
      for (let i = enemies.length - 1; i >= 0; i--) {
          ...
          if (checkCollision(player, enemy)) {
              let contactDamage = enemy.contactDamage * damageReduction * finalDamageMultiplier;
              if (isSuperShieldActive) contactDamage *= 0.1;
              applyDamage(contactDamage);
              createExplosion(enemy.position, enemy.size);
              enemies.splice(i, 1);
          }
      }
  
      if (boss && checkCollision(player, boss)) {
          let bossContactDamage = boss.contactDamage * damageReduction * finalDamageMultiplier;
          if (isSuperShieldActive) bossContactDamage *= 0.1;
          applyDamage(bossContactDamage);
          createExplosion(player.position, player.size);
      }
      ...
      for (let i = enemyProjectiles.length-1; i>=0; i--) {
          ...
          if (checkCollision(player, p)) {
              if (isSuperShieldActive) {
                  const shieldToRegen = player.maxShield * 0.02;
                  player.shield = Math.min(player.maxShield, player.shield + shieldToRegen);
              } else {
                  applyDamage(p.damage * damageReduction * finalDamageMultiplier);
              }
              createExplosion(p.position, p.size);
      ...
      for (let i = collectibles.length - 1; i >= 0; i--) {
          ...
          if (checkCollision(player, collectible)) {
              if (collectible.type === 'revivalKey') {
                  player.revivalKeys++;
              }
      ...
      if (xpToAdd > 0) player.xp += xpToAdd * difficultyModifiers.xp;
      if (coinsToAdd > 0) player.coins += coinsToAdd;
      ...
      if (gameTime >= nextRevivalKeySpawnTime) {
        collectibles.push({ id: `key_${now}`, type: 'revivalKey', position: { x: Math.random() * (C.GAME_WIDTH - C.REVIVAL_KEY_SIZE.width), y: -C.REVIVAL_KEY_SIZE.height }, size: C.REVIVAL_KEY_SIZE, velocity: { x: 0, y: C.REVIVAL_KEY_SPEED } });
        nextRevivalKeySpawnTime += C.REVIVAL_KEY_SPAWN_INTERVAL_MS;
      }

      explosions = explosions.map(ex => ({ ...ex, life: ex.life + 1 })).filter(ex => ex.life < ex.maxLife);
      
      if (player.hull <= 0 && !isInvincible) {
          if (player.revivalKeys > 0) {
              setIsReviving(true);
              setRevivalTimeLeft(C.REVIVE_SCREEN_DURATION_MS);
          } else {
              triggerGameOver();
          }
      }

      return { ...prev, player, enemies, projectiles, enemyProjectiles, collectibles, boss, explosions, score: score + scoreToAdd, phase, gameTime, maxEnemies, enemySpawnRateMultiplier, enemyFireRateMultiplier, helper, nextBossSpawnTime, nextRevivalKeySpawnTime };
    });
    gameLoopRef.current = requestAnimationFrame(gameLoop);
  }, [isLevelingUp, isReviving, difficulty, quality, SKILLS_CONFIG, controlMode, triggerGameOver]);
  
  useEffect(() => {
    let timerId: number;
    if (isReviving && !isPaused) {
        if (revivalTimeLeft <= 0) {
            triggerGameOver();
        } else {
            timerId = window.setTimeout(() => {
                setRevivalTimeLeft(t => t - 100);
            }, 100);
        }
    }
    return () => clearTimeout(timerId);
  }, [isReviving, isPaused, revivalTimeLeft, triggerGameOver]);

  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
        if (e.key === 'Escape' && !isLevelingUp && !isReviving) { e.preventDefault(); setIsPaused(p => !p); } 
    ...
    const handleBlur = () => !isLevelingUp && !isReviving && setIsPaused(true);
    ...
    if (isPaused || isLevelingUp || isReviving) {
        ...
        if (pauseStartTime.current !== null) {
            ...
            setGameState(g => ({ ...g, enemies: g.enemies.map(e => ({...e, lastShotTime: e.lastShotTime + pauseDuration})), boss: g.boss ? {...g.boss, lastShotTime: g.boss.lastShotTime + pauseDuration} : null, helper: g.helper ? {...g.helper, lastShotTime: g.helper.lastShotTime + pauseDuration, life: g.helper.life + pauseDuration} : null, player: {...g.player, specialAbilities: {...g.player.specialAbilities, bomb: {...g.player.specialAbilities.bomb, lastUsed: g.player.specialAbilities.bomb.lastUsed+pauseDuration}, powerShot: {...g.player.specialAbilities.powerShot, lastUsed: g.player.specialAbilities.powerShot.lastUsed+pauseDuration}, helper: {...g.player.specialAbilities.helper, lastUsed: g.player.specialAbilities.helper.lastUsed+pauseDuration}, superShield: {...g.player.specialAbilities.superShield, lastUsed: g.player.specialAbilities.superShield.lastUsed+pauseDuration}}, activeSpecial: g.player.activeSpecial ? {...g.player.activeSpecial, until: g.player.activeSpecial.until + pauseDuration} : null, invincibleUntil: g.player.invincibleUntil > 0 ? g.player.invincibleUntil + pauseDuration : 0 }, nextBossSpawnTime: g.nextBossSpawnTime + pauseDuration, nextRevivalKeySpawnTime: g.nextRevivalKeySpawnTime + pauseDuration }));
  ...
  }, [gameLoop, isPaused, isLevelingUp, isReviving, handleActivateSpecial, controlMode]);

  const { player, enemies, projectiles, enemyProjectiles, boss, explosions, score, gameTime, helper, phase, collectibles } = gameState;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {collectibles.map(c => <CollectibleVisual key={c.id} collectible={c} quality={quality} />)}
        ...
        {isReviving && <RevivalScreen timeLeft={revivalTimeLeft} onRevive={handleUseRevivalKey} onDecline={triggerGameOver} />}
